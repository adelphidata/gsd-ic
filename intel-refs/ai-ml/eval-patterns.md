...
**Avoiding overstatement:** Point estimates without confidence intervals overstate precision. Prototype evaluation results generalize to operational conditions only with explicit qualification. Use ICD 203 Words of Estimative Probability (WEP) when extrapolating beyond test conditions — "likely" (roughly 55%–80% confidence) for claims supported by reasonable evidence; drop to "roughly even chance" (45%–55%) when the claim requires operational assumptions that have not been validated in the evaluation window. Do not use "highly likely" for any claim that depends on infrastructure scaling or operational environment changes not tested in the evaluation. See `intel-refs/tradecraft/words-of-estimative-probability.md` for the canonical 7-band table.
